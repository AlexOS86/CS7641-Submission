...
if __name__ == '__main__':
    ...
    datasets = []
    dataset1_details = {
            'data': loader.SpamDataMlOrg(verbose=verbose, seed=seed),
            'name': 'Spam',
            'readable_name': 'Spam',
            'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [0.1],
                               'NN__hidden_layer_sizes': [(114,)], 'NN__learning_rate_init': [0.016]}
        }
    dataset2_details = {
            'data': loader.Phishing(verbose=verbose, seed=seed),
            'name': 'phish',
            'readable_name': 'Phishing',
            'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [1.00E-06],
                               'NN__hidden_layer_sizes': [(60, 60, 60)], 'NN__learning_rate_init': [0.016]}
    }
    if args.dataset1:
